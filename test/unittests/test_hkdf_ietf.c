...
/*
 * Test HKDF functionality (RFC 5869).
 */
...
/* See RFC 5869, Appendix A.  Assume SHA-256 is used. */
struct test_case_hkdf
{
    unsigned        test_case_no;
    unsigned char   ikm[100];
    size_t          ikm_len;
    unsigned char   salt[100];
    size_t          salt_len;
    unsigned char   info[100];
    size_t          info_len;
    unsigned char   prk[100];
    size_t          prk_len;
...
static const struct test_case_hkdf tests[] =
{
...

    {
        ...
        .info_len = 80,
        .prk =
            "\x06\xa6\xb8\x8c\x58\x53\x36\x1a\x06\x10\x4c\x9c\xeb\x35\xb4\x5c"
...
static void
test_hkdf (const struct test_case_hkdf *test)
{
    unsigned char okm[ test->okm_len ];
    unsigned char prk[100];
    unsigned prk_len;

    lsquic_hkdf_extract(EVP_sha256(), test->ikm, test->ikm_len, test->salt, test->salt_len,
                        prk, &prk_len);
    assert(prk_len == test->prk_len);
    assert(0 == memcmp(prk, test->prk, prk_len));

    lsquic_hkdf_expand(EVP_sha256(), test->prk, test->prk_len, test->info, test->info_len,
                       okm, sizeof(okm));
    assert(0 == memcmp(okm, test->okm, sizeof(okm)));
}
