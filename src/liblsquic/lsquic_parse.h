...
struct parse_funcs
{
    ...
    /*
     * If `size' is zero, `fin' must be non-zero.  If `fin' is non-zero,
     * `size' must be zero.
     *
     * While IETF QUIC (at least as of ID-11) allows zero-size STREAM frames
     * when FIN is not set, our code does not produce them.
     *
     * Return number of bytes written to `buf'.  If an error occurs, a
...
extern const struct parse_funcs lsquic_parse_funcs_id12;
