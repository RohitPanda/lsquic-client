...
#define CLIENT_LABEL "client hs"
#define CLIENT_LABEL_SZ (sizeof(CLIENT_LABEL) - 1)
#define SERVER_LABEL "server hs"
#define SERVER_LABEL_SZ (sizeof(SERVER_LABEL) - 1)

void
lsquic_hkdf_extract (const struct env_md_st *, const unsigned char *ikm,
             unsigned ikm_len, const unsigned char *salt, unsigned salt_len,
             unsigned char *prk, unsigned *prk_len);

void
lsquic_hkdf_expand (const struct env_md_st *, const unsigned char *prk_secret,
