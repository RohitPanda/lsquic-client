...
stream_write_to_packet_hsk (struct frame_gen_ctx *fg_ctx, const size_t size)
{
    ...
    st = write_stream_frame(fg_ctx, size, packet_out);
...
stream_write_to_packet_std (struct frame_gen_ctx *fg_ctx, const size_t size)
{
    ...
    if (packet_out)
        return write_stream_frame(fg_ctx, size, packet_out);
    else
        return SWTP_STOP;
}
                         ...
                         size_t thresh)
{
    ...
    enum swtp_status (*write_to_packet)(struct frame_gen_ctx *, const size_t);

    if (!stream_is_hsk(stream))
        write_to_packet = stream_write_to_packet_std;
    else
        write_to_packet = stream_write_to_packet_hsk;

    seen_ok = 0;
    while ((size = frame_gen_size(&fg_ctx), thresh ? size >= thresh : size > 0)
           || frame_gen_fin(&fg_ctx))
    {
        switch (write_to_packet(&fg_ctx, size))
        {
        case SWTP_OK:
            if (!seen_ok++)
