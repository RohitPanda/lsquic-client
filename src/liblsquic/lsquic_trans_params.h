...
/* [draft-ietf-quic-transport], Section 6.4 */
enum transport_param_id
{
    TPI_INITIAL_MAX_STREAM_DATA     =  0,
    TPI_INITIAL_MAX_DATA            =  1,
    TPI_INITIAL_MAX_BIDI_STREAMS    =  2,
    TPI_IDLE_TIMEOUT                =  3,
    TPI_PREFERRED_ADDRESS           =  4,
    TPI_MAX_PACKET_SIZE             =  5,
    TPI_STATELESS_RESET_TOKEN       =  6,
    TPI_ACK_DELAY_EXPONENT          =  7,
    TPI_INITIAL_MAX_UNI_STREAMS     =  8,
};

#define IQUIC_REQUIRED_TRANSPORT_PARAMS             ( \
    (1 << TPI_INITIAL_MAX_STREAM_DATA)              | \
    (1 << TPI_INITIAL_MAX_DATA)                     | \
    (1 << TPI_IDLE_TIMEOUT))
...
struct transport_params
{
    ...
    uint32_t    tp_init_max_stream_data;
