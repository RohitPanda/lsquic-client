...
struct ietf_full_conn
{
    ...
    lsquic_packno_t             ifc_max_ack_packno;
...
ietf_full_conn_init (struct ietf_full_conn *conn,
           ...
           unsigned flags)
{
    struct lsquic_stream *crypto_stream;
    ...
    lsquic_alarmset_init_alarm(&conn->ifc_alset, AL_ACK, ack_alarm_expired, conn);
    ...
    lsquic_rechist_init(&conn->ifc_rechist, LSQUIC_LOG_CONN_ID, 1);
    ...
    crypto_stream = lsquic_stream_new(0, &conn->ifc_pub,
        &lsquic_cry_sm_if, &conn->ifc_crypto_driver, 0, 0,
        SCF_IETF|SCF_DI_AUTOSWITCH);
    if (!crypto_stream)
        goto err1;
    if (!lsquic_hash_insert(conn->ifc_pub.all_streams, &crypto_stream->id,
                                sizeof(crypto_stream->id), crypto_stream))
        goto err2;
    conn->ifc_crypto_driver.csd_conn     = &conn->ifc_conn;
    conn->ifc_crypto_driver.csd_stream   = crypto_stream;

    conn->ifc_conn.cn_if = ietf_full_conn_iface_ptr;
    conn->ifc_flags = flags | IFC_CREATED_OK;
    conn->ifc_max_ack_packno = IQUIC_INVALID_PACKNO;
    return 0;

  err2:
    lsquic_stream_destroy(crypto_stream);
  err1:
           ...
           const char *hostname, unsigned short max_packet_size, int is_ipv4)
{
    ...
    /* TODO: check retval */
                conn->ifc_conn.cn_esf.i->esfi_create_client(conn->ifc_enpub,
                                        &conn->ifc_conn, &conn->ifc_ver_neg);
    ...
    lsquic_stream_call_on_new(conn->ifc_crypto_driver.csd_stream);
...
static void
reset_ack_state (struct ietf_full_conn *conn)
{
    LSQ_WARN("%s: TODO", __func__);   /* TODO */
}
...
ietf_full_conn_ci_handshake_ok (struct lsquic_conn *lconn)
{
                             ...
                             el = lsquic_hash_next(conn->ifc_pub.all_streams))
    {
        ...
        if (0 != lsquic_stream_set_max_send_off(stream,
                                            params.tp_init_max_stream_data))
        {
            ABORT_WARN("cannot set peer-supplied max_stream_data=%"PRIu32
                "on stream %"PRIu64, params.tp_init_max_stream_data,
                stream->id);
            return;
        }
    }

    conn->ifc_cfg.max_stream_send = params.tp_init_max_stream_data;
...
{
    if (conn->ifc_crypto_driver.csd_stream->stream_flags & STREAM_WANT_READ)
        lsquic_stream_dispatch_read_events(conn->ifc_crypto_driver.csd_stream);
}
...
{
    if (conn->ifc_crypto_driver.csd_stream->stream_flags & STREAM_WANT_WRITE)
        lsquic_stream_dispatch_write_events(conn->ifc_crypto_driver.csd_stream);
}
...
write_is_possible (struct ietf_full_conn *conn)
{
    ...
    packet_out = lsquic_send_ctl_last_scheduled(&conn->ifc_send_ctl);
             ...
             lsquic_time_t received)
{
    ...
    if (0 == lsquic_send_ctl_got_ack(&conn->ifc_send_ctl, acki, received))
    {
        if (lsquic_send_ctl_largest_ack2ed(&conn->ifc_send_ctl))
            lsquic_rechist_stop_wait(&conn->ifc_rechist,
            ...
            enum stream_ctor_flags flags)
{
    ...
    flags |= SCF_ALLOW_OVERLAP|SCF_DI_AUTOSWITCH|SCF_IETF;
    ...
    struct lsquic_packet_in *packet_in, const unsigned char *p, size_t len)
{
    ...
    if (is_valid_packno(conn->ifc_max_ack_packno) &&
                            packet_in->pi_packno <= conn->ifc_max_ack_packno)
    {
        LSQ_DEBUG("Ignore old ack (max %"PRIu64")", conn->ifc_max_ack_packno);
        return parsed_len;
    }

    EV_LOG_ACK_FRAME_IN(LSQUIC_LOG_CONN_ID, new_acki);
    conn->ifc_max_ack_packno = packet_in->pi_packno;
                                        ...
                                        struct lsquic_packet_in *packet_in)
{
    ...
    max_packno = lsquic_rechist_largest_packno(&conn->ifc_rechist);
                                        ...
                                        struct lsquic_packet_in *packet_in)
{
    ...
    st = lsquic_rechist_received(&conn->ifc_rechist, packet_in->pi_packno,
